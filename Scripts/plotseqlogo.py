...
import numpy as np
from concise.preprocessing.sequence import AMINO_ACIDS, DNA, RNA
from concise.utils.letters import all_letters
from descartes.patch import Polygon, PolygonPatch, PolygonPath
...
def standardize_polygons_str(data_str):
    ...
    for path_str in path_strs:
        data = np.array([tuple(map(float, x.split())) for x in path_str.strip().split(",")])
    ...
    polygons = []
    for data in polygons_data:
        polygons.append(load_wkt("POLYGON((%s))" % ",".join(" ".join(map(str, x)) for x in data)))
...
VOCABS = {
    "DNA": OrderedDict([("A", "red"), ("C", "blue"), ("G", "orange"), ("T", "green")]),
    "RNA": OrderedDict([("A", "red"), ("C", "blue"), ("G", "orange"), ("U", "green")]),
    "AA": OrderedDict(
        [
            ("A", "#CCFF00"),
            ("B", "orange"),
    ...
    "RNAStruct": OrderedDict([("P", "red"), ("H", "green"), ("I", "blue"), ("M", "orange"), ("E", "violet")]),
}
