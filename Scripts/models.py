...
class NaiveNet(nn.Module):
    ...
    def __init__(self, input_size=None, num_task=None):
        ...
        self.SharedFC = nn.Sequential(
            nn.Linear(in_features=128 * in_features_3, out_features=1024), nn.ReLU(), nn.Dropout()
        )
...
class NaiveNet_v2(nn.Module):
    ...
    def __init__(self, input_size=None, num_task=None):
        ...
        self.Flatten = nn.Flatten()
        self.SharedFC = nn.Sequential(
            nn.Linear(in_features=in_features_3 * 256, out_features=1024), nn.ReLU(), nn.Dropout()
        )
        for i in range(num_task):
            setattr(
                self,
