...
def train(model, train_loader, test_loader, args):
    ...
    logwriter = csv.DictWriter(
        logfile, fieldnames=["epoch", "loss", "val_loss", "val_acc", "val_precision", "val_recall"]
    )
    ...
    for epoch in range(args.epochs):
        ...
        for i, (x, y_true) in enumerate(train_loader):
            ...
            if i == 1:
                # sanity check y_pred
                print(
                    "Sanity Checking, at epoch%02d, iter%02d, y_pred is" % (epoch, i),
        ...
        val_loss, metrics_dict = test(
            model, test_loader, loss_weight, args.use_embedding, args.use_uncertain_weighting, MutiTaskLoss
        )
        logwriter.writerow(
            dict(
                epoch=epoch,
                loss=training_loss.cpu().numpy() / len(train_loader.dataset),
        ...
        print(
            "===>Epoch %02d: loss=%.5f, val_loss=%.4f, val_acc=%.4f,\
...
if __name__ == "__main__":
    ...
    parser.add_argument("--grad_norm", default=False, type=str2bool, nargs="?", help="activate grad norm")
    parser.add_argument("--mode", default="train", help="Set the model to train or not")
    parser.add_argument("--use_embedding", default=False, type=str2bool, nargs="?", help="activate embedding")
    parser.add_argument(
        "--use_uncertain_weighting", default=False, type=str2bool, nargs="?", help="activate uncertain weighting"
    )
    ...
    if args.mode == "train":
        # load data
        train_loader, valid_loader = load_RM_data(
            args.inputs,
        ...
        test_data = RMdata(data_path=args.inputs, length=args.length, use_embedding=args.use_embedding, mode=args.mode)
