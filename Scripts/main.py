...
from util_att import cal_attention, evaluate
from util_funs import cutseqs, highest_x, seq2index, str2bool, visualize
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--att_window", type=int, default=3, help="length of sliding window to aggregate attention weights"
    )
    parser.add_argument(
        "--verbose", type=str2bool, default=False, help="whether showing detailed prediction results or not"
    )
    parser.add_argument(
        "--save", type=str2bool, default=False, help="save the prob, p-value, predicted label and attention matrix"
    )
    ...
    if args.save:
        ...
        pd.DataFrame(data=probs, index=RMs, columns=cols_name).to_csv(
            os.path.join(args.save_path, args.save_id, "../Results/probs.csv"), header=True
        )
        pd.DataFrame(data=p_values, index=RMs, columns=cols_name).to_csv(
            os.path.join(args.save_path, args.save_id, "../Results/p_values.csv"), header=True
        )
        pd.DataFrame(data=labels, index=RMs, columns=cols_name_2).to_csv(
            os.path.join(args.save_path, args.save_id, "../Results/pred_labels.csv"), header=True
        )
        pd.DataFrame(data=attention, index=RMs, columns=cols_name_2).to_csv(
            os.path.join(args.save_path, args.save_id, "../Results/attention.csv"), header=True
        )
